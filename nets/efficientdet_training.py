...
if __name__=='__main__':
    ...
    y = np.array([[0,0,244,124,0],[34,25,100,100,1]])
    boxes = np.array(y[:, :4], dtype=np.float32)
    boxes[:, 0] = boxes[:, 0] / 640
    boxes[:, 1] = boxes[:, 1] / 640
    boxes[:, 2] = boxes[:, 2] / 640
    boxes[:, 3] = boxes[:, 3] / 640
    one_hot_label = np.eye(2)[np.array(y[:, 4], np.int32)]

    y = np.concatenate([boxes, one_hot_label], axis=-1)
    assignment = bbox_util.assign_boxes(y)
    regression = assignment[:, :5]
    classification = assignment[:, 5:]
    target0 = np.reshape(regression, [-1, 5])
    target1 = np.reshape(classification, [-1, 3])
    print(target1.shape)
    print(target0.shape)